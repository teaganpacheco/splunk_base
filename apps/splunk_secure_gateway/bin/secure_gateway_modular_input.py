...
async def _periodic_spacebridge_server_update(websocket,
    ...
    while not websocket.closed:
        ...
        if current_spacebridge_server != conf_file_spacebridge_server:
            try:
...
class SpacebridgeModularInput(BaseModularInput):
    ...
    def do_run(self, input_config):
        ...
        # Enable the ssg_enable_modular_input so we run it at least once and ensure any new modular_inputs are enabled
        self.logger.info('Enable ssg_enable_modular_input')
        splunk_service.enable_ssg_mod_input("ssg_enable_modular_input", self.session_key)
