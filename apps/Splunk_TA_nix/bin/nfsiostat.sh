...
if [ "$KERNEL" = "Linux" ] ; then
    ...
    # For CentOS and RHEL
        ...
        #For  RHEL 8.5, RHEL 8.4 and RHEL 8.3
        if [ -e $OS_FILE ] && ( ( (awk -F'=' '/ID=/ {print $2}' $OS_FILE | grep -q rhel) && (awk -F'=' '/VERSION_ID=/ {print $2}' $OS_FILE | grep -Eq 8.6\|8.5\|8.4\|8.3\|9) ) || ( (awk -F'=' '/ID=/ {print $2}' $OS_FILE | grep -q cent) && (awk -F'=' '/VERSION_ID=/ {print $2}' $OS_FILE | grep -Eq 8) ) );
        then
            # shellcheck disable=SC2016
            FORMAT='{
                if (NR%10==2){
